...
- **GitHub:**
  - **Режим репозитория:** введи `owner/repo` (напр. `tiangolo/fastapi`), затем на следующем шаге отправь `-` (минус), чтобы пропустить топики.
  - **Режим поиска по топикам:** введи `-` (минус) в поле репозитория, затем укажи список тегов (каждый с новой строки), например:
    `llm`
    `python`
    Бот будет искать обновлённые репозитории по этим топикам.
